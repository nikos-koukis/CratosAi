#!/usr/bin/env bash
# Issues the knowledge service's DEVELOPMENT certificates from the Vault's
...
#   server.pem, server-key.pem   service listener (localhost, 127.0.0.1)
...
issue server knowledge serverAuth "DNS:localhost,IP:127.0.0.1"
issue orchestrator orchestrator clientAuth "URI:spiffe://$TRUST_DOMAIN/orchestrator"
rm -f ca.srl

echo "Knowledge service development certificates written to $(pwd) (valid $DAYS days)."
