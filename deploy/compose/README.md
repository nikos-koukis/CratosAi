...
- **PostgreSQL:**
  ...
  - Each service gets its own role and database (`vault`, `mcp_router`, `orchestrator`, `app_api`, `dashboard`, `audit`). A role owns only its database and cannot connect to the others or to `postgres`.
